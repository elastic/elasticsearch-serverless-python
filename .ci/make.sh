...
if [[ "$CMD" == "codegen" ]]; then
  docker run \
     ...
     cd /code/elastic-client-generator-python && GIT_BRANCH=$VERSION python -m nox -s generate-es-serverless && \
     cd /code/elasticsearch-serverless-python && python -m nox -s format"
