...
When you create your serverless {es} cluster for the first time you'll be provided an API key.
...
es.options(
    api_key=("another-api-key-id", "another-api-key-secret")
).indices.get(index="*")
...
# Adds the HTTP header 'Authorization: ApiKey <base64 api-key-id:api-key-secret>'
es = Elasticsearch(
    "https://localhost:9200",
    ca_certs="/path/to/http_ca.crt",
    api_key=("api-key-id", "api-key-secret")
